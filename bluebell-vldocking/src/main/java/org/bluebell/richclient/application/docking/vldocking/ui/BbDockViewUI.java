...
/**
 * A UI for the dock view that uses a widget like style.
 * <p />
 ...
 */
public class BbDockViewUI extends DockViewUI implements ActivationAware {

    /**
     * The singleton UI instance.
     */
    private static final BbDockViewUI INSTANCE = new BbDockViewUI();
    ...
    public void installUI(JComponent c) {
        ...
        // (JAF), 20100226, maximized dockables lose their background color so set it explicitly (for instance under
        // Substance 5.3)
    ...
    protected final void installBorder(DockView dockView, Boolean active) {
        ...
        if (parent instanceof TabbedDockableContainer) {
            ...
            border = UIManager.getBorder(VLDockingUtils.activationKey("DockView.maximizedDockableBorder", active));
        } else if (parent instanceof JPanel) { // BbFloatingDialog
            border = UIManager.getBorder(VLDockingUtils.activationKey("DockView.singleDockableBorder", active));
        } else {
            border = UIManager.getBorder(VLDockingUtils.activationKey("DockView.maximizedDockableBorder", active));
        }
